...
def get_price_min_tx(code, end_date=None, count=10, frequency='1d'):  # 分钟线获取
    ...
    df.set_index(['time'], inplace=True)
    df.index.name = ''  # 处理索引
    #df['close'].iloc[-1] = float(st['data'][code]['qt'][code][3])  # 最新基金数据是3位的
...
# -------------------------------------------------------------------------
# 以下为自定义函数，以上为原生函数
def get_increase(code, name = '未知'):
    df_day = get_price(code, frequency='1d', count=1)
    df_min = get_price(code, frequency='1m', count=1)
    close_day = df_day['close'].values[0]
    close_min = df_min['close'].values[0]
    close_day = float(close_day)
    close_min = float(close_min)
    try:
        growth_percentage = ((close_min - close_day) / close_day) * 100
        print(f"code: {code} name：{name}") # 打印股票代码和名称
        print(f"close: {close_day}") # 打印日线收盘价
        print(f"current: {close_min}") # 打印当前分钟线收盘价
        print(f"increase: {growth_percentage:.2f}%") # 打印增长百分比
    except ZeroDivisionError:
        print("日线的收盘价为0，无法计算增长百分比。")

# 免费港股历史数据接口
# 查看恒生指数历史数据：https://tsanghi.com/api/fin/index/HKG/daily?token=demo&ticker=HSI
# 查看恒生科技指数历史数据：https://tsanghi.com/api/fin/index/HKG/daily?token=demo&ticker=HSTECH
